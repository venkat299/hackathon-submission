...
def health_issues_process(env, state):
    ...
    while True:
        ...
        # First, check if a currently active issue should be resolved
        ...
        # Then, check if the member is in a cooldown period from a previous issue
        if 'issue_cooldown_until' in state.narrative_flags and state.current_day < state.narrative_flags['issue_cooldown_until']:
            continue

        # --- Define Base Probabilities (chance per day) ---
        PROBS = {
            "Minor Illness (Cold/Flu)": 0.005,
            "Muscle Strain/Joint Pain": 0.004,
            "Bout of Indigestion": 0.007,
            "Stress Headache": 0.006,
            "Blood Pressure Spike": 0.003
        }

        # --- Calculate Risk Modifiers based on current state ---
        ...
        for issue, base_prob in PROBS.items():
            if random.random() < (base_prob + risk_modifiers[issue]):
                ...
                log_event(state, "HEALTH_ISSUE", "SIM_CORE", {
                    "issue": issue,
                    "triggering_factors": {
                        "is_traveling": state.logistics.is_traveling,
                        "recovery_score": recovery,
                        "adherence": state.intervention_plan.adherence_status
                    },
                    "duration_days": duration
                })
                break # Trigger only one issue per day
...
def proactive_expert_process(env, state, elyx_agents):
    ...
    while True:
        ...
        # --- Corrected: Added proactive triggers for all experts ---
        # Note: Using 'if/elif' to prevent multiple proactive messages on the same day.

        # Trigger for Dr. Warren: Comments on new lab results (High Priority)
        if state.health_data.lab_results and abs(state.current_day - state.health_data.lab_results.get('last_test_day', -999)) < 1:
            trigger_event = "Proactive Follow-up: New lab results are in. Let's discuss the findings."
            responder = "Dr. Warren"
        
        # Trigger for Advik: Checks for low wearable recovery score
        elif state.health_data.wearable_stream.get("recovery_score", 100) < 30:
            trigger_event = "Proactive Check-in: Noticed the recovery score is very low."
            responder = "Advik"
...
def state_update_process(env, state):
    ...
    while True:
        ...
        base_hrv = 45 + (state.current_day / 30)
        ...
        state.health_data.wearable_stream['recovery_score'] = max(0, min(100, round(base_recovery + recovery_noise)))